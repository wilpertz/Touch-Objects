...
setoutletassist(0, "Outlet 0: Event message [name/index state] for [route]");
setoutletassist(1, "Outlet 1: Activity gate / Signum (1 if any on, 0 if all off)");
...
function output_state(changed_index) {
  ...
  try {
    if (changed_index !== undefined && changed_index >= 0 && changed_index < count) {
      ...
      outlet(0, [tag, states[changed_index]]);
    } else {
